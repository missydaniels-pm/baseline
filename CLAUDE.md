...
- **Trigger** — episode trigger dimension (hybrid model). `user_id` nullable: **NULL = curated global seed** (shared, admin-curated, capped at 12 via `SEED_TRIGGERS`; editing that list adds new globals on next deploy but never renames/removes existing ones — globals are soft-retired via `is_active`); non-null = a user's custom trigger. `name` (100), `is_active`, `created_at`. Uniqueness enforced by two DB **partial** unique indexes (`ux_trigger_global_name` = unique `lower(name)` where `user_id IS NULL`; `ux_trigger_user_name` = unique `(user_id, lower(name))` where `user_id IS NOT NULL`) — a plain composite index wouldn't constrain globals since NULLs compare distinct. Custom "deletion" will be a soft-deactivate (`is_active=False`), never hard-delete, so linked history survives. **Episode capture path shipped (7/15/26):** the episode form (new + edit) has a chip multi-select picker (active globals + the user's active customs) with an inline "+ add"; the write helper `_resolve_trigger()` does match-and-link (active global → own custom, reactivating if soft-deactivated → else create, guarded by a `begin_nested` savepoint against the concurrent same-name race), and `_save_episode_triggers_from_form()` persists `EpisodeTrigger` links (user-scoped, dedup'd, replace-on-save on edit). Episode history shows linked triggers (`⚡ …`). **AI check-in parity shipped (7/15/26):** the check-in prompt lists the user's triggers and returns a `triggers` array; `_apply_ai_triggers()` links names that match an existing trigger with `source='ai'` (via match-only `_match_trigger()`, never creating), and returns unmatched names as *suggestions* — the AI never auto-creates a trigger. Suggestions ride in the Flask session (signed cookie, keyed by episode id), the check-in reply names them + the "Episode logged" tag links to the episode, and `edit_episode` pre-loads them as pre-checked "suggested" chips; **Save confirms** them (created `source='user'`). **Manage Triggers shipped (7/15/26):** `/triggers` (linked from Settings, not in nav) — rename + pause/resume your own customs (soft-deactivate preserves history), globals shown read-only. Mirrors the `/symptoms` management CRUD; rename rejects collisions with a global or another own custom (Symptom unique-name precedent, no merge in v1). `test_trigger_manage.py`. **Trigger work is now complete for the monolith** except: **v2 per-user hide of globals** and **dashboard trigger breakdowns — both deferred to post-rebuild** (React redoes the viz; per-user hide isn't needed while the global list is ≤12).
- **EpisodeTrigger** — junction linking an episode to a Trigger (global or custom); unique `(episode_id, trigger_id)`. Episode-side `cascade='all, delete-orphan'` (same pattern as EpisodeIntervention), so `db.session.delete(episode)` cleans up links. `source` ('user' | 'ai', default 'user') records provenance — the episode form writes 'user'; a future AI check-in path will write 'ai'. Editing the episode form re-links as 'user' (a user review is a confirmation).
- **Protocol** — preventative protocols with name, start_date, dose, frequency, status, and optional `why` ("Why I'm doing this" — powers personalized compliance messaging). Also `med_class` (nullable, **episode-diary Layer A** — drug class for step-therapy docs; no CHECK, vocabulary ratified with the React capture UI; unwritten in the monolith).
- **ProtocolEvent** — status/dose-change log per protocol (`event_type`, `detail`, back-datable `date`, `created_at`). **Episode-diary Layer A** added `stop_reason` (nullable, CHECK-scoped to `stopped`/`paused` events **and** the controlled list `ineffective|side_effects|cost|doctors_advice|other`, constraint `protocol_events_stop_reason_check`) + `stop_reason_note` (nullable text). `detail` is deliberately **not** reused for the reason — `assess_experiment` already writes it on a status event. Both columns unwritten in the monolith (capture = Layer B, React).
...
**AI check-in:** Opt-in. Uses load_dotenv(override=True) to ensure .env always wins over shell environment. Returns structured JSON parsed into episode/compliance records. **Onset = classify-and-resolve (tz Increment 3, 7/17/26):** the model does NOT compute a date — it returns `onset_time_expr` (verbatim time phrase) + `onset_time_type` (enum), and deterministic `resolve_onset()` resolves it against the message anchor (browser `client_time` else `user_now()`) via `dateparser` + a colloquial-normalization layer. Never logs a future onset; vague/unresolvable → message time + an "adjust the time" nudge through the existing episode edit form. Triggers follow a **suggest-and-confirm** rule (unlike interventions, which the AI auto-creates): matched existing triggers link automatically (`source='ai'`), but a trigger name the AI doesn't recognize is never created — it's surfaced as a suggestion the user confirms by saving on the episode form (created `source='user'`). Owner decision 7/15/26.
...
**Foreign keys / ON DELETE:** all 20 FKs carry DB-level directives, declared once in `database.EXPECTED_FK_ONDELETE` (16 CASCADE, 3 SET NULL) — that dict is the single source of truth for the model kwargs, the PostgreSQL migration, and `verify_fk_ondelete()`. **Both increments shipped 8/8/26** — Inc 1 added the constraints, Inc 2 removed the hand-ordered cleanup and added `passive_deletes=True`, so a hard delete is one `db.session.delete(parent)` and the DB cascades. `dev_reset` is the documented exception (keeps the User row). **The constraints are load-bearing — do not reintroduce hand-ordered child cleanup.** Adding an owned table means: right `ondelete=`, an `EXPECTED_FK_ONDELETE` entry, and `passive_deletes=True` on the relationship — nothing to add to `delete_account`. `check_fk_orphans.py` (read-only pre-flight) and `verify_fk_ondelete()` must both come back clean before migrating any new environment. SQLite is dev-only and cannot alter constraints in place — a stale local schema is caught by the startup check; fix by deleting `instance/migraine_tracker.db`. See CONVENTIONS.md.
